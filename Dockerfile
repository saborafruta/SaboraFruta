...
EXPOSE 8000
CMD if [ "${SERVICE_ROLE:-web}" = "worker" ]; then python worker_health.py & exec celery -A config worker --beat --scheduler django_celery_beat.schedulers:DatabaseScheduler --loglevel=INFO --concurrency=1 --uid=nobody --gid=nogroup; else python manage.py migrate --fake-initial --noinput && python manage.py migrate_tenant_databases --retry-errors --continue-on-error && python manage.py ensure_quality_schema && exec gunicorn config.wsgi:application --bind 0.0.0.0:$PORT --workers 2 --timeout 120; fi
