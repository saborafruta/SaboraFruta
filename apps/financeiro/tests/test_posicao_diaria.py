...
class PosicaoDiariaCaixaTests(TestCase):
    ...
    def test_taxa_de_recebimento_reduz_entrada_e_exibe_bruto(self):
        ...
        self.assertContains(relatorio, ".cr-detail { display:inline; color:#172033; }")
        self.assertNotContains(relatorio, "color:#607086")
        self.assertNotContains(relatorio, "color:#718096")
    ...
    def test_admin_exclui_pagamento_de_venda_da_posicao_diaria(self):
        ...
        pagamento_bom = PagamentoVendaPDV.objects.create(
            venda_pdv=venda, forma_pagamento=self.forma, conta_bancaria=self.banco,
            valor=Decimal("80.00"),
        )
        ...
        response = self.client.post(reverse("financeiro:posicao_diaria"), {
            "acao": "excluir_pagamento_venda", "movimento_id": pagamento_errado.pk,
            "data_referencia": "2026-08-21", "justificativa": "Pagamento lancado em duplicidade",
        })

        self.assertEqual(response.status_code, 302)
        pagamento_errado.refresh_from_db()
        venda.refresh_from_db()
        ...
        self.assertEqual(self.banco.saldo_atual, Decimal("178.20"))

        listagem = self.client.get(reverse("financeiro:posicao_diaria"), {"data": "2026-08-21"})
        registros_venda = [
            mov for mov in listagem.context["posicao"]["entradas"]
            if mov.origem_codigo == "venda" and mov.venda_pdv_id == venda.pk
        ]
        self.assertEqual(len(registros_venda), 1)
        self.assertEqual(registros_venda[0].registro_id, pagamento_bom.pk)

    def test_nao_admin_nao_pode_excluir_pagamento_de_venda(self):
        outro_perfil = PerfilAcesso.objects.create(empresa=self.empresa, nome="Operador", is_admin=False)
        operador = Usuario.objects.create_user(
            email="operador@inoovated.com", nome="Operador Caixa", password="teste1234",
            empresa=self.empresa, filial=self.filial, perfil=outro_perfil,
        )
        venda = VendaPDV.objects.create(
            filial=self.filial, numero_venda=91, status="finalizada",
            valor_total=Decimal("80.00"), valor_pago=Decimal("80.00"), usuario=self.usuario,
            data_venda=datetime(2026, 8, 21, 12, tzinfo=timezone.get_current_timezone()),
        )
        pagamento = PagamentoVendaPDV.objects.create(
        ...
        response = self.client.post(reverse("financeiro:posicao_diaria"), {
            "acao": "excluir_pagamento_venda", "movimento_id": pagamento.pk,
            "data_referencia": "2026-08-21", "justificativa": "Tentativa sem permissao",
        })

        self.assertEqual(response.status_code, 302)
        pagamento.refresh_from_db()
        self.assertNotEqual(pagamento.status, "excluido")
