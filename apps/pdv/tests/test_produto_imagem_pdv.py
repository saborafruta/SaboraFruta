...
class ProdutoImagemPDVTests(SimpleTestCase):
    ...
    def test_modal_identifica_tipo_campanha_validade_e_brinde(self):
        ...
        self.assertIn('referenciaModalOferta(p)', template)
        self.assertIn('condicaoComboOferta(p)', template)
        self.assertNotIn("['kit','combo','categoria','brinde'].includes(p.tipo) && p.detalhe", template)
