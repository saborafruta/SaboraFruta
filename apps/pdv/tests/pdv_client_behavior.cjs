...
(async () => {
  ...
  const exactCombos=create();
  const exactOffer={...comboProduct.ofertas[0],quantidade_exata:true};
  exactCombos._pushItem(comboProduct,exactOffer,true);
  const comboItem=exactCombos.venda.itens[0];
  assert.equal(exactCombos.quantidadePasso(comboItem),3);
  exactCombos.alterarQuantidade(comboItem,exactCombos.quantidadePasso(comboItem));
  assert.equal(comboItem.quantidade,6,'Plus adds one complete combo');
  assert.equal(comboItem.valor_total,259.94);
  exactCombos.alterarProdutoCatalogo(comboProduct,-1);
  assert.equal(comboItem.quantidade,3,'Catalog minus removes one combo');
  exactCombos._pushItem(comboProduct,exactOffer,true);
  assert.equal(exactCombos.venda.itens.length,1,'Repeated selection merges the same combo');
  assert.equal(comboItem.quantidade,6);
  comboItem.quantidade=9;
  exactCombos.atualizarQuantidadeItem(comboItem);
  assert.equal(comboItem.valor_total,389.91,'Direct input supports three complete combos');
  comboItem.quantidade=7;
  exactCombos.atualizarQuantidadeItem(comboItem);
  assert.equal(comboItem.quantidade,6,'Direct input is normalized to complete combos');
  const resumed=JSON.parse(JSON.stringify(comboItem));
  exactCombos.alterarQuantidade(resumed,exactCombos.quantidadePasso(resumed));
  assert.equal(resumed.quantidade,9,'Persisted combo context keeps quantity increments');
  assert.equal(exactCombos.normalizarQuantidade(0,comboItem),3);
  exactCombos.alterarQuantidade(comboItem,-3);
  exactCombos.alterarQuantidade(comboItem,-3);
  assert.equal(exactCombos.venda.itens.length,0,'Minus on the last combo removes it');
  ...
  const allocation=create();
  allocation.venda.itens=[{valor_total:29.99},{valor_total:29.99},{valor_total:29.99},{valor_total:0}];
  allocation.venda.desconto=9.97;allocation.recalcularTotais();
  assert.equal(allocation.impactoDescontoVenda(allocation.venda.itens[0]).total,26.66);
  assert.equal(allocation.impactoDescontoVenda(allocation.venda.itens[1]).total,26.67);
  assert.equal(allocation.impactoDescontoVenda(allocation.venda.itens[2]).total,26.67);
  assert.equal(allocation.impactoDescontoVenda(allocation.venda.itens[3]).desconto,0);
  assert.equal(allocation.rateioDescontoVenda.reduce((s,l)=>s+Math.round(l.total*100),0),8000,'Rounding must close the sale exactly');
  assert.equal(allocation.venda.itens[0].valor_total,29.99,'Allocation is display-only');
  allocation.venda.itens=[{_id:1,valor_total:20},{_id:2,valor_total:80}];allocation.venda.desconto=20;allocation.recalcularTotais();
  assert.equal(allocation.impactoDescontoVenda(allocation.venda.itens[0]).total,16);
  assert.equal(allocation.impactoDescontoVenda(allocation.venda.itens[1]).total,64);
  allocation.venda.acrescimo=5;allocation.recalcularTotais();
  assert.equal(allocation.venda.total,85);
  assert.equal(allocation.rateioDescontoVenda.reduce((s,l)=>s+l.total,0),80,'Surcharge is not distributed as item discount');
  allocation.venda.desconto=100;allocation.recalcularTotais();
  assert.equal(allocation.rateioDescontoVenda.every(l=>l.total===0),true);
  allocation.venda.desconto=0;allocation.recalcularTotais();
  assert.equal(allocation.impactoDescontoVenda(allocation.venda.itens[1]).total,80);
  allocation.removerItem(allocation.venda.itens[0]);
  assert.equal(allocation.rateioDescontoVenda.length,1);
  const inlineTotal=create();
  inlineTotal.venda.itens=[{quantidade:3,valor_total:89.97,desconto_valor:10}];
  inlineTotal.recalcularTotais();
  inlineTotal.iniciarTotalCarrinho();
  assert.equal(inlineTotal.totalCarrinhoRascunho,'89,97');
  inlineTotal.totalCarrinhoRascunho='80,00';
  assert.equal(inlineTotal.venda.total,89.97,'Typing must not apply a partial discount');
  inlineTotal.aplicarTotalCarrinho();
  assert.equal(inlineTotal.venda.total,80);
  assert.equal(inlineTotal.venda.desconto,9.97);
  assert.equal(inlineTotal.descontoGeralRascunho.percentual,'11,08');
  assert.equal(inlineTotal.showModalDesconto,false,'Inline total never opens the discount modal');
  assert.equal(inlineTotal.venda.itens[0].desconto_valor,10,'Keep item discounts untouched');
  for(const invalid of ['', ' ', 'abc', 'Infinity', '-1', '90', '1e1']) {
    inlineTotal.iniciarTotalCarrinho();inlineTotal.totalCarrinhoRascunho=invalid;inlineTotal.aplicarTotalCarrinho();
    assert.equal(inlineTotal.venda.total,80,'Invalid total must preserve the existing discount');
  }
  inlineTotal.iniciarTotalCarrinho();inlineTotal.totalCarrinhoRascunho='10';
  inlineTotal.editandoTotalCarrinho=false;inlineTotal.aplicarTotalCarrinho();
  assert.equal(inlineTotal.venda.total,80,'Escape cancels without applying on blur');
  inlineTotal.iniciarTotalCarrinho();inlineTotal.totalCarrinhoRascunho='0';inlineTotal.aplicarTotalCarrinho();
  assert.equal(inlineTotal.venda.total,0,'A full discount is allowed');
  inlineTotal.iniciarTotalCarrinho();inlineTotal.totalCarrinhoRascunho='89,97';inlineTotal.aplicarTotalCarrinho();
  assert.equal(inlineTotal.venda.desconto,0,'Restoring the total removes the general discount');
  inlineTotal.venda.acrescimo=5;inlineTotal.venda.pagamentos=[{valor:30}];inlineTotal.recalcularTotais();
  inlineTotal.iniciarTotalCarrinho();inlineTotal.totalCarrinhoRascunho='80';inlineTotal.aplicarTotalCarrinho();
  assert.equal(inlineTotal.venda.total,80);
  assert.equal(inlineTotal.venda.desconto,14.97);
  assert.equal(inlineTotal.venda.restante,50);
  inlineTotal.iniciarTotalCarrinho();inlineTotal.totalCarrinhoRascunho='0';inlineTotal.aplicarTotalCarrinho();
  assert.equal(inlineTotal.venda.total,80,'Surcharges cannot be discounted by the inline field');
  inlineTotal.iniciarTotalCarrinho();inlineTotal.totalCarrinhoRascunho='10';
  inlineTotal.venda.itens[0].valor_total=100;inlineTotal.recalcularTotais();inlineTotal.aplicarTotalCarrinho();
  assert.equal(inlineTotal.venda.total,90.03,'A changed cart invalidates an old total draft');
  inlineTotal.venda.itens=[];inlineTotal.recalcularTotais();inlineTotal.iniciarTotalCarrinho();
  assert.equal(inlineTotal.editandoTotalCarrinho,false);
